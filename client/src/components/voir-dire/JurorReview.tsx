...
export function JurorReview({
  ...
  const [showNotesWalkthrough, setShowNotesWalkthrough] = useState(false);
    ...
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 h-full flex flex-col relative">
      <div className="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-end shrink-0 gap-4">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            onClick={() => setShowNotesWalkthrough(true)}
            data-testid="button-notes-walkthrough"
            className="inline-flex items-center px-4 py-2 text-sm font-semibold rounded-lg transition-colors bg-amber-50 text-amber-700 border border-amber-200 hover:bg-amber-100"
          >
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="group relative">
                    <Eye className="w-3.5 h-3.5 text-blue-500 cursor-help" />
                    <div className="absolute bottom-full right-0 mb-1.5 w-56 p-2.5 bg-blue-900 text-blue-100 text-[10px] rounded-lg shadow-lg opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50">
                      Your notes are used by the AI to generate more accurate risk assessments. Even brief observations about demeanor or body language help distinguish between jurors who gave similar verbal responses.
                      <div className="absolute top-full right-3 w-2 h-2 bg-blue-900 rotate-45 -mt-1" />
                    </div>
                  </div>
                </div>
                <div className="flex flex-wrap gap-1.5 mb-2">
                  {DEMEANOR_TAGS.map(tag => {
                    const currentNotes = selectedJuror.notes || '';
                    const isApplied = currentNotes.includes(`[${tag}]`) || currentNotes.toLowerCase().includes(tag.toLowerCase());
                    return (
                      <button
                        key={tag}
                        onClick={() => {
                          const tagStr = `[${tag}]`;
                          const newNotes = currentNotes ? `${currentNotes} ${tagStr}` : tagStr;
                          onUpdateJuror(selectedJuror.number, { notes: newNotes });
                          setSelectedJuror({ ...selectedJuror, notes: newNotes });
                        }}
                        disabled={isApplied}
                        data-testid={`button-note-tag-${tag.toLowerCase().replace(/\s+/g, '-')}-${selectedJuror.number}`}
                        className={`px-2 py-0.5 text-[10px] rounded-full border transition-colors ${
                          isApplied
                            ? 'bg-amber-50 text-amber-600 border-amber-200 cursor-default opacity-60'
                            : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-amber-50 hover:text-amber-700 hover:border-amber-300 cursor-pointer'
                        }`}
                      >
                        {tag}
                      </button>
                ...
                placeholder="Observations about demeanor, body language, tone, interactions with other jurors..."
                ...
                data-testid={`textarea-notes-${selectedJuror.number}`} />
      ...
      {showNotesWalkthrough && (() => {
        const sortedJurors = [...jurors].sort((a, b) => a.number - b.number);
        const currentJuror = sortedJurors[walkthroughIdx];
        if (!currentJuror) return null;
        const jurorResponses = responses.filter(r => r.jurorNumber === currentJuror.number);
        const hasNotes = (currentJuror.notes || '').trim().length > 0;
        const notesCount = sortedJurors.filter(j => (j.notes || '').trim().length > 0).length;

        return (
          <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowNotesWalkthrough(false)}>
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              onClick={(e) => e.stopPropagation()}
              className="bg-white rounded-2xl shadow-2xl max-w-xl w-full max-h-[85vh] overflow-hidden flex flex-col"
              data-testid="notes-walkthrough-modal"
            >
              <div className="p-5 border-b border-slate-200 flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-bold text-slate-900">Notes Review</h3>
                  <p className="text-xs text-slate-500 mt-0.5">{notesCount} of {sortedJurors.length} jurors have notes</p>
                </div>
                <button onClick={() => setShowNotesWalkthrough(false)} className="p-1 rounded hover:bg-slate-100">
              ...
              <div className="flex-1 overflow-y-auto p-5">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <span className={`w-8 h-8 rounded-lg flex items-center justify-center text-sm font-bold ${hasNotes ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-500'}`}>
                      {currentJuror.number}
                    </span>
                    <div>
                      <span className="font-semibold text-slate-900">{currentJuror.name || `Juror #${currentJuror.number}`}</span>
                      {currentJuror.occupation && (
                        <span className="text-xs text-slate-400 ml-2">{currentJuror.occupation}</span>
                      )}
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <button
                      onClick={() => setWalkthroughIdx(Math.max(0, walkthroughIdx - 1))}
                    ...
                    <span className="text-xs font-medium text-slate-400 min-w-[60px] text-center">
                      {walkthroughIdx + 1} / {sortedJurors.length}
                    </span>
                    <button
                      onClick={() => setWalkthroughIdx(Math.min(sortedJurors.length - 1, walkthroughIdx + 1))}
                      disabled={walkthroughIdx === sortedJurors.length - 1}
                      className="p-1.5 rounded-lg hover:bg-slate-100 disabled:opacity-30 disabled:cursor-not-allowed"
                ...
                {jurorResponses.length > 0 && (
                  <div className="bg-slate-50 rounded-xl border border-slate-200 p-3 mb-4">
                    <span className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider">Recent Responses</span>
                    <div className="mt-1.5 space-y-1 max-h-28 overflow-y-auto">
                      {jurorResponses.slice(-4).map((r, i) => (
                        <div key={i} className="text-xs text-slate-600">
                          {r.responseText.startsWith('[') ? (
                            <ReactionText text={r.responseText} className="text-xs" />
                          ) : (
                            <span>"{r.responseText.slice(0, 120)}{r.responseText.length > 120 ? '...' : ''}"</span>
                          )}
                        </div>
                      ))}
                    </div>
                ...
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {DEMEANOR_TAGS.map(tag => {
                    const currentNotes = currentJuror.notes || '';
                    const isApplied = currentNotes.includes(`[${tag}]`);
                    return (
                      <button
                        key={tag}
                        onClick={() => {
                          const tagStr = `[${tag}]`;
                          const newNotes = currentNotes ? `${currentNotes} ${tagStr}` : tagStr;
                          onUpdateJuror(currentJuror.number, { notes: newNotes });
                        }}
                        disabled={isApplied}
                        data-testid={`button-wt-tag-${tag.toLowerCase().replace(/\s+/g, '-')}`}
                        className={`px-2 py-1 text-xs rounded-full border transition-colors ${
                          isApplied
                            ? 'bg-amber-50 text-amber-600 border-amber-200 opacity-60 cursor-default'
                            : 'bg-slate-50 text-slate-600 border-slate-200 hover:bg-amber-50 hover:text-amber-700 hover:border-amber-300 cursor-pointer'
                        }`}
                      >
                        {tag}
                ...
                <textarea
                  value={currentJuror.notes || ''}
                  onChange={(e) => onUpdateJuror(currentJuror.number, { notes: e.target.value })}
                  placeholder="Observations about demeanor, body language, tone, interactions..."
                  className="w-full p-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-white text-sm resize-none min-h-[100px] outline-none"
                ...
                <div className="mt-2 p-2 bg-blue-50 rounded-lg border border-blue-200">
                  <p className="text-[10px] text-blue-600">
                    ...
                    Notes feed directly into AI analysis — even "seemed nervous" or "avoided eye contact" meaningfully improves risk assessment accuracy.
                  </p>
                </div>
              </div>

              <div className="p-4 border-t border-slate-200 flex justify-between items-center">
                <span className="text-xs text-slate-400">{notesCount} jurors with notes</span>
                <button
                  onClick={() => setShowNotesWalkthrough(false)}
