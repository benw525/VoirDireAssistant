...
- **AI**: Anthropic Claude Opus 5 (`claude-opus-5`) for high-stakes legal reasoning (juror analysis, brief summary, strike-for-cause, Batson, voir dire generation/refinement); Claude Sonnet 5 (`claude-sonnet-5`) for live follow-up suggestions; OpenAI gpt-4o-mini for in-app chat assistant; OpenAI whisper-1 for voice transcription; Google Gemini 3.1 Pro Preview (`gemini-3.1-pro-preview`) for strike list OCR/parsing, with `gemini-3.5-flash` as fallback (also auto-falls back on 404 if the primary preview model is rotated/removed) with image support via sharp conversion. Shared Anthropic helper at `server/anthropic.ts` (claudeComplete / claudeJson). Note: `temperature` is unsupported/deprecated on opus-4-7 and the 5-series Claude models; the helper omits it automatically for those models.
...
- Follow-up suggestions generated via `POST /api/collab/suggest-followups` (Claude Sonnet 5) — questioner does NOT see raw juror responses
