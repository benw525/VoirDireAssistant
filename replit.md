...
- **AI**: Anthropic Claude Opus 4.7 for high-stakes legal reasoning (juror analysis, brief summary, strike-for-cause, Batson, voir dire generation/refinement); Claude Sonnet 4.6 for live follow-up suggestions; OpenAI gpt-4o-mini for in-app chat assistant; OpenAI whisper-1 for voice transcription; Google Gemini 3.1 Pro for strike list OCR/parsing with image support via sharp conversion. Shared Anthropic helper at `server/anthropic.ts` (claudeComplete / claudeJson). Note: Opus 4.7 deprecates the `temperature` parameter, helper omits it automatically for that model.
